...
export const useUserStore = defineStore('userStore', {
    ...
    actions: {
        ...
        autoLigon () {
            ...
            if (token) {
                axiosR.post(`/auth/auto-login`, { "token": token }).then((res) => {
                    this.user = res.data;
                }).catch(() => {
        ...
        userUpdateInfo (data) {
            axiosR.post(`/user/info`, data).then((res) => {
                this.user = res.data.user;
                if(res.data.token !== ""){
                    localStorage.setItem('token', res.data.token);
                }
        ...
        userChangePassword (data) {
            axiosR.post(`/user/change-password`, data).then(() => {
            }).catch(err => {
                this.error = err.response.data;
            });            
        },
        logout () {
            axiosR.get('/auth/logout'            )
                .then((res) => {
                    if (res.status === 200 ){
                        localStorage.removeItem('token')
                        this.user = null; 
        ...
        userAddAvatar (data) {
            axiosR.post(`/user/avatar/add`, data).then((res) => {
        ...
        userRemoveAvatar () {
            axiosR.get(`/user/avatar/remove`).then((res) => {
                if(res.status === 200){
